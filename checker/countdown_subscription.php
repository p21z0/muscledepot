    ...
    $days_remaining = floor($remaining / 86400) +2;
        ...
        if ($days_remaining>7) {
            ...
            if (!($reminder_status==$status_change_checker)){
                ...
                $subscription_data2=get_where_custom($table_name, $column, $subscription_id);
                foreach ($subscription_data2 as $key => $row) {
                    $reminder_status=$row['reminder_status'];
                    include ("../mailing/send_subscription_notice.php");
                }
            } else {
